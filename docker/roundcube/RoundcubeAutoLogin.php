...
class RoundcubeAutoLogin
{
    ...
    public function redirect()
    {
        ...
        // header('Location: ' . 'http://127.0.0.1:8000/redirect/' . '?task=mail');
        header('Location: /roundcube/?task=mail');
    }
    ...
    private function _get_token()
    {
        ...
        // curl_setopt($this->ch, CURLOPT_URL, "http://127.0.0.1:80/");
        curl_setopt($this->ch, CURLOPT_RETURNTRANSFER, TRUE);
        curl_setopt($this->ch, CURLOPT_COOKIEFILE, '');
        curl_setopt($this->ch, CURLOPT_COOKIEJAR, '');
        $response = curl_exec($this->ch);
...
<form action="http://domain.com/roundcube/RoundcubeAutoLogin.php" method="post" name="autologin">
  UserID <input name="_user" type="text" id="_user">
  Passwort <input name="_pass" type="password" id="_pass">
...
$rcuser = $_REQUEST['_user'];
$rcpass = $_REQUEST['_pass'];


// set your roundcube domain path
// $rc = new RoundcubeAutoLogin('http://127.0.0.1:8000/roundcube/');
$rc = new RoundcubeAutoLogin('http://127.0.0.1:80/');
